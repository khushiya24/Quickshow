...
const Navbar = () => {
  ...
  const { user } = useUser();
  const { openSignIn } = useClerk(); // ✅ FIXED (was OpenSignIn)
  ...
  return (
    <div className="fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5 bg-transparent backdrop-blur">
      ...
      <div
        ...
        z-50 flex flex-col md:flex-row items-center max-md:justify-center 
        ...
        ${isOpen ? "max-md:w-full px-10" : "max-md:w-0 px-0"}`}
      >
        {/* Close Icon Mobile */}
        <XIcon
